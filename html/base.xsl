...
<xsl:template name="quicklinks">
    <ul class="quicklinks">
      ...
      <li><a href="?verb=ListRecords&amp;metadataPrefix=oai_dc">ListRecords (DC)</a> | </li>
      <li><a href="?verb=ListRecords&amp;metadataPrefix=oai_lom">ListRecords (LOM)</a> | </li>
...
<xsl:template match="oai:header">
  ...
  <table class="values">
    ...
    <td class="value">
      ...
      <xsl:text> </xsl:text><a class="link" href="?verb=GetRecord&amp;metadataPrefix=oai_lom&amp;identifier={oai:identifier}">oai_lom</a>
...
<xsl:template match="oai:setSpec">
  ...
  <td class="value"><xsl:value-of select="."/>
    ...
    <xsl:text> </xsl:text><a class="link" href="?verb=ListRecords&amp;metadataPrefix=oai_dc&amp;set={.}">Records(DC)</a>
    <xsl:text> </xsl:text><a class="link" href="?verb=ListRecords&amp;metadataPrefix=oai_lom&amp;set={.}">Records(LOM)</a>
  </td></tr>
</xsl:template>
...
<tr><td class="key">Source</td><td class="value"><a href="{.}"><xsl:value-of select="."/></a></td></tr></xsl:template>
...
<xsl:template match="oai_lom:lom"  xmlns:oai_lom="http://ltsc.ieee.org/xsd/LOM" >
  <div class="dcdata">
    <h3>Learning Object Metadata (oai_lom)</h3>
    <table class="dcdata">
      <xsl:apply-templates select="*" />
    </table>
  </div>
...
<xsl:template match="lom:general" xmlns:lom="http://ltsc.ieee.org/xsd/LOM">
<xsl:for-each select='lom:identifier'>
	<tr><td class="key">General.Identifier.catalog</td><td class="value"><xsl:value-of select="lom:catalog"/></td></tr>
	<tr><td class="key">General.Identifier.entry</td><td class="value"><xsl:value-of select="lom:entry"/></td></tr>
</xsl:for-each>
<tr><td class="key">General.Title</td><td class="value"><xsl:value-of select="lom:title"/></td></tr>
<xsl:for-each select='lom:description'>
	<tr><td class="key">General.Description</td><td class="value"><xsl:value-of select="."/></td></tr>
</xsl:for-each>
<tr><td class="key">General.Keywords</td><td class="value">	
	<xsl:for-each select='lom:keyword'>
		<xsl:value-of select="."/>; 
	</xsl:for-each>
</td></tr>
<xsl:for-each select='lom:coverage'>
	<tr><td class="key">General.Coverage</td><td class="value"><xsl:value-of select="."/></td></tr>
</xsl:for-each>
<xsl:for-each select='lom:language'>
	<tr><td class="key">General.Language</td><td class="value"><xsl:value-of select="."/></td></tr>
</xsl:for-each>
</xsl:template>

<xsl:template match="lom:lifeCycle" xmlns:lom="http://ltsc.ieee.org/xsd/LOM">
<xsl:for-each select='lom:contribute'>
	<tr><td class="key">LifeCycle.Contribute</td><td class="value">
		<xsl:value-of select="lom:role/lom:value"/>: <xsl:value-of select="lom:entity"/>
	</td></tr>	
</xsl:for-each>
</xsl:template>

<xsl:template match="lom:educational" xmlns:lom="http://ltsc.ieee.org/xsd/LOM">
<xsl:for-each select='lom:learningResourceType'>
	<tr><td class="key">Educational.LearningResourceType</td><td class="value"><xsl:value-of select="lom:value"/></td></tr>
</xsl:for-each>
</xsl:template>

<xsl:template match="lom:technical" xmlns:lom="http://ltsc.ieee.org/xsd/LOM">
<xsl:for-each select='lom:location'>
	<tr><td class="key">Technical.location</td><td class="value"><a href="{.}"><xsl:value-of select="."/></a></td></tr>
</xsl:for-each>
<xsl:for-each select='lom:thumbnail'>
	<tr><td class="key">Technical.thumbnail</td><td class="value">
	<xsl:choose>
    	<xsl:when test='starts-with(.,"http" )'>	
			<a href="{.}"><xsl:value-of select="."/></a>
		</xsl:when>
...
<xsl:template match="lom:rights" xmlns:lom="http://ltsc.ieee.org/xsd/LOM">
<xsl:for-each select='lom:description'>
	<tr><td class="key">Rights</td><td class="value"><xsl:value-of select="."/></td></tr>
</xsl:for-each>
</xsl:template>

<xsl:template match="lom:classification" xmlns:lom="http://ltsc.ieee.org/xsd/LOM">
<tr><td class="key">Classification</td><td class="value"><xsl:value-of select="."/></td></tr></xsl:template>

<xsl:template match="lom:metaMetadata" xmlns:lom="http://ltsc.ieee.org/xsd/LOM">
<tr><td class="key">MetaMetadata</td><td class="value"><xsl:value-of select="lom:identifier/lom:entry"/>@<xsl:value-of select="lom:identifier/lom:catalog"/></td></tr></xsl:template>

<xsl:template match="lom:relation" xmlns:lom="http://ltsc.ieee.org/xsd/LOM">
<tr><td class="key">Relation</td><td class="value">
  <xsl:choose>
    <xsl:when test='starts-with(lom:resource/lom:description/lom:string,"http" )'>
      <xsl:choose>
        <xsl:when test='string-length(lom:resource/lom:description/lom:string) &gt; 50'>
          <a class="link" href="{lom:resource/lom:description/lom:string}">URL</a>
          <i> URL not shown as it is very long.</i>
        </xsl:when>
        <xsl:otherwise>
          <a href="{lom:resource/lom:description/lom:string}"><xsl:value-of select="lom:resource/lom:description/lom:string"/></a>
        </xsl:otherwise>
      </xsl:choose>
    </xsl:when>
    <xsl:otherwise>
      <xsl:value-of select="lom:resource/lom:description/lom:string"/>
    </xsl:otherwise>
  </xsl:choose>
</td></tr>
</xsl:template>
